...
const calculate_nesting_depth = (source) => {
  ...
  if (!source.includes('{')) {
    ...
    for (const line of lines) {
      ...
      const depth = Math.floor(leadingSpaces / 4) || Math.floor(leadingSpaces / 2);
...
const get_complexity_rating = (cyclomatic) => {
  if (cyclomatic <= 5) return { rating: 'low', color: '#4caf50', label: 'Simple' };
  if (cyclomatic <= 10) return { rating: 'moderate', color: '#ff9800', label: 'Moderate' };
  if (cyclomatic <= 20) return { rating: 'high', color: '#f44336', label: 'Complex' };
...
const calculate_aggregate_complexity = (entities) => {
  ...
  const total_cyclomatic = complexities.reduce((sum, c) => sum + c.cyclomatic, 0);
  const total_loc = complexities.reduce((sum, c) => sum + c.loc, 0);
  const total_nesting = complexities.reduce((sum, c) => sum + c.nesting_depth, 0);
  ...
  return {
    ...
    max_cyclomatic: Math.max(...complexities.map((c) => c.cyclomatic)),
    avg_loc: Math.round((total_loc / total_functions) * 10) / 10,
    total_loc,
    avg_nesting_depth: Math.round((total_nesting / total_functions) * 10) / 10,
    max_nesting_depth: Math.max(...complexities.map((c) => c.nesting_depth)),
