...
/**
 * @fileoverview Tool definitions and JSON schemas for MCP server tools.
 * Contains string representations and schemas for function/project-related tool calls.
 * @module lib/strings
 */

/**
 * Tool definitions for the MCP server.
 * Each tool contains a name, description, and JSON schema for its response format.
 * @type {Object.<string, {name: string, description: string, parameters?: Object, response?: Object}>}
 */
const tools = {
  function_list: {
    name: 'function_list',
  ...
  function_caller_tree: {
    name: 'function_caller_tree',
    description: `Retrieves a tree of functions that call the specified function, recursively up to a specified depth.

     The \`depth\` parameter controls how many levels of callers to retrieve:
     - depth=1 (default): returns only direct callers
     - depth=2: returns direct callers and their callers
     - depth=-1: returns all callers recursively (use with caution for large codebases)

     The tree structure includes loop detection to prevent infinite recursion.

     The returned JSON will be a tree structure where each node contains:
     - id: unique identifier of the function
     - symbol: function name
     - filename: file containing the function
     - start_line/end_line: location in the file
     - parameters: function parameters
     - return_type: return type if available
     - comment: function documentation
     - call_line: line number where the call occurs (for child nodes)
     - callers: array of caller nodes (recursive structure)
     - loop: true if this node was already visited (prevents infinite recursion)
     - truncated: true if depth limit was reached`
  },
  function_callee_tree: {
    name: 'function_callee_tree',
    description: `Retrieves a tree of functions called by the specified function, recursively up to a specified depth.

     The \`depth\` parameter controls how many levels of callees to retrieve:
     - depth=1 (default): returns only direct callees
     - depth=2: returns direct callees and what they call
     - depth=-1: returns all callees recursively (use with caution for large codebases)
     ...
     - comment: function documentation
     - call_line: line number where the call occurs (for child nodes)
     - callees: array of callee nodes (recursive structure)
     - loop: true if this node was already visited (prevents infinite recursion)
     - truncated: true if depth limit was reached`
  },
  entity_references: {
    name: 'entity_references',
    description: `Retrieves all code locations where a struct or class is referenced.

     This tool is useful for understanding how a struct/class type is used throughout
     the codebase. It tracks:
     - variable: Variable declarations using the type (e.g., \`struct Point p;\`)
     - parameter: Function parameters using the type (e.g., \`void foo(struct Point p)\`)
     - field: Struct field declarations using the type
     - typedef: Type definitions referencing the type
     - macro: #define macros that reference the type (C/C++ only)

     The returned JSON will be an array of references:

     {
       ...
       "properties": {
         ...
         "entity_id": {
           "type": "number",
           "description": "The ID of the referenced entity (struct/class)"
         },
         "symbol": {
           "type": "string",
           "description": "The name of the referenced struct/class"
         },
         "entity_type": {
           "type": "string",
           "description": "The type of entity (struct or class)"
         },
         "filename": {
           "type": "string",
           "description": "The file containing the reference"
         },
         "line": {
           "type": "number",
           "description": "The line number of the reference"
         },
         "reference_type": {
           "type": "string",
           "enum": ["variable", "parameter", "field", "typedef", "macro"],
