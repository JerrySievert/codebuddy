...
const calculate_similarity = (a, b) => {
  const tokens_a = new Set(
    a.split(/\s+/).filter(function is_significant_token(t) {
      return t.length > 2;
    })
  );
  const tokens_b = new Set(
    b.split(/\s+/).filter(function is_significant_token(t) {
      return t.length > 2;
    })
  );
  ...
  const intersection = new Set(
    [...tokens_a].filter(function is_in_both_sets(x) {
      return tokens_b.has(x);
    })
  );
...
const detect_code_duplication = async (
  ...
  const normalized_functions = functions.map(
    function normalize_function_source(fn) {
      return {
        ...fn,
  ...
  for (let i = 0; i < normalized_functions.length; i++) {
    ...
    if (clones.length > 0) {
      ...
      duplicates.push({
        ...
        clones,
        total_duplicated_lines: clones.reduce(function sum_clone_lines(sum, c) {
          return sum + c.lines;
        }, 0)
      });
    }
