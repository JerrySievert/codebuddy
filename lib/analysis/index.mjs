...
const detect_dead_code = async (project_id) => {
  ...
  const is_likely_entry_point = function check_if_likely_entry_point(symbol) {
    return entry_point_patterns.some(function matches_entry_pattern(pattern) {
  ...
  const total_dead_lines = dead_functions.reduce(function sum_dead_lines(
    sum,
    fn
  ) {
    return sum + fn.lines;
...
/**
 * Extract tokens from normalized source code.
 * Tokens are significant words (length > 2) used for similarity comparison.
 * @param {string} normalized_source - The normalized source code
 * @returns {Set<string>} Set of tokens
 */
const extract_tokens = (normalized_source) => {
  if (!normalized_source) return new Set();
  return new Set(
    normalized_source.split(/\s+/).filter(function is_significant_token(t) {
      return t.length > 2;
    })
  );
};
...
/**
 * Calculate Jaccard similarity between two pre-computed token sets.
 * @param {Set<string>} tokens_a - First token set
 * @param {Set<string>} tokens_b - Second token set
 * @returns {number} Similarity score between 0 and 1
 */
const calculate_similarity_from_tokens = (tokens_a, tokens_b) => {
  if (tokens_a.size === 0 && tokens_b.size === 0) return 1;
  if (tokens_a.size === 0 || tokens_b.size === 0) return 0;

  // Count intersection size without creating a new Set
  let intersection_size = 0;
  const smaller = tokens_a.size <= tokens_b.size ? tokens_a : tokens_b;
  const larger = tokens_a.size <= tokens_b.size ? tokens_b : tokens_a;
  ...
  const union_size = tokens_a.size + tokens_b.size - intersection_size;

  return intersection_size / union_size;
};

/**
 * Check if two token sets could possibly meet the similarity threshold.
 * Uses the mathematical property that Jaccard(A,B) <= min(|A|,|B|) / max(|A|,|B|).
 * This allows us to skip comparisons that can't possibly meet the threshold.
 * @param {number} size_a - Size of first token set
 * @param {number} size_b - Size of second token set
 * @param {number} threshold - Similarity threshold
 * @returns {boolean} True if similarity could meet threshold
 */
const could_meet_threshold = (size_a, size_b, threshold) => {
  if (size_a === 0 || size_b === 0) return false;
  const min_size = Math.min(size_a, size_b);
  const max_size = Math.max(size_a, size_b);
  // Maximum possible Jaccard similarity given the set sizes
  // Occurs when smaller set is a subset of larger set
  const max_possible_similarity = min_size / max_size;
  return max_possible_similarity >= threshold;
};

/**
 * Generate a simple hash signature for bucketing similar functions.
 * Functions with different signatures are unlikely to be similar.
 * @param {Set<string>} tokens - Token set
 * @param {number} lines - Number of lines in function
 * @returns {string} Hash signature for bucketing
 */
const generate_bucket_key = (tokens, lines) => {
  // Bucket by approximate token count (rounded to nearest 5) and line count (rounded to nearest 10)
  const token_bucket = Math.floor(tokens.size / 5) * 5;
  const line_bucket = Math.floor(lines / 10) * 10;
  return `${token_bucket}_${line_bucket}`;
};

/**
 * Simple hash function for MinHash.
 * @param {string} str - String to hash
 * @param {number} seed - Seed for the hash
 * @returns {number} Hash value
 */
const simple_hash = (str, seed) => {
  let hash = seed;
 ...
 * MinHash allows fast estimation of Jaccard similarity.
 * @param {Set<string>} tokens - Token set
 * @param {number} num_hashes - Number of hash functions (signature length)
 * @returns {number[]} MinHash signature
 */
const generate_minhash_signature = (tokens, num_hashes = 64) => {
  if (tokens.size === 0) return new Array(num_hashes).fill(Infinity);

  const signature = new Array(num_hashes).fill(Infinity);
  const tokens_array = Array.from(tokens);

  for (let h = 0; h < num_hashes; h++) {
    for (const token of tokens_array) {
      const hash_val = simple_hash(token, h * 31337);
 ...
 * @param {number[]} sig_a - First signature
 * @param {number[]} sig_b - Second signature
 * @returns {number} Estimated similarity (0-1)
 */
const estimate_similarity_from_minhash = (sig_a, sig_b) => {
  let matches = 0;
  for (let i = 0; i < sig_a.length; i++) {
    if (sig_a[i] === sig_b[i]) matches++;
  }
  return matches / sig_a.length;
};
 ...
 * 1. Pre-compute tokens once during normalization (avoids repeated tokenization)
 * 2. Bucket functions by approximate size to reduce comparison space
 * 3. Early exit using token count bounds (mathematical impossibility check)
 * 4. Optimized intersection counting without creating new Sets
 *
 * @param {number} project_id - The project ID to analyze
...
const detect_code_duplication = async (
  ...
  // Phase 1: Normalize all sources, pre-compute tokens and MinHash signatures (O(n))
  // MinHash signatures enable fast similarity estimation without full set comparison
  const use_minhash = functions.length > 1000; // Only use MinHash for large projects
  const normalized_functions = functions.map(
    function normalize_and_tokenize(fn) {
      const normalized_source = normalize_source(fn.source);
      const tokens = extract_tokens(normalized_source);
      return {
        ...fn,
        normalized_source,
        tokens,
        token_count: tokens.size,
        minhash: use_minhash ? generate_minhash_signature(tokens, 64) : null
      };
    }
  );

  // Phase 2: Group functions into buckets by approximate size (O(n))
  // Functions in different buckets are unlikely to be similar
  const buckets = new Map();
  for (const fn of normalized_functions) {
    const key = generate_bucket_key(fn.tokens, fn.lines);
    if (!buckets.has(key)) {
      buckets.set(key, []);
    }
    buckets.get(key).push(fn);

    // Also add to adjacent buckets to catch edge cases
    // This handles functions on bucket boundaries
    const adjacent_keys = [
      generate_bucket_key(fn.tokens, fn.lines + 10),
      generate_bucket_key(fn.tokens, fn.lines - 10)
    ];
    for (const adj_key of adjacent_keys) {
      if (adj_key !== key) {
        if (!buckets.has(adj_key)) {
          buckets.set(adj_key, []);
        }
        // Mark as cross-reference only (to avoid duplicate comparisons)
        buckets.get(adj_key).push({ ...fn, is_adjacent: true });
      }
    }
  }

  // Phase 3: Compare functions within each bucket (reduces O(n²) significantly)
  let comparisons_made = 0;
  let comparisons_skipped_by_threshold = 0;
  let comparisons_skipped_by_lines = 0;
  let comparisons_skipped_by_minhash = 0;

  for (const [bucket_key, bucket_functions] of buckets) {
    // Only process primary functions (not adjacent references)
    const primary_functions = bucket_functions.filter((fn) => !fn.is_adjacent);

    for (let i = 0; i < primary_functions.length; i++) {
      const fn_a = primary_functions[i];
      if (processed.has(fn_a.id)) continue;

      const clones = [];

      // Compare with all functions in the bucket (including adjacent refs)
      for (let j = 0; j < bucket_functions.length; j++) {
        const fn_b = bucket_functions[j];

        // Skip self-comparison and already processed
        if (fn_b.id === fn_a.id || processed.has(fn_b.id)) continue;

        // Skip if comparing primary to primary and j <= i (already compared)
        if (!fn_b.is_adjacent && fn_b.id <= fn_a.id) continue;
        ...
        if (
          ...
          comparisons_skipped_by_lines++;
          continue;
        }

        // Early exit: check if similarity could possibly meet threshold
        // based on token set sizes alone
        if (
          ...
          comparisons_skipped_by_threshold++;
          continue;
        }

        // MinHash pre-filter: use fast signature comparison to skip unlikely matches
        // This is probabilistic but much faster than full token comparison
        if (use_minhash && fn_a.minhash && fn_b.minhash) {
          const estimated_sim = estimate_similarity_from_minhash(
            fn_a.minhash,
            fn_b.minhash
          );
          // Use a lower threshold for the estimate to avoid false negatives
          // MinHash can underestimate by ~10-15% for small sets
          if (estimated_sim < similarity_threshold - 0.15) {
            comparisons_skipped_by_minhash++;
            continue;
          }
        }
        ...
        // Use pre-computed token sets for exact similarity calculation
        const similarity = calculate_similarity_from_tokens(
          fn_a.tokens,
          fn_b.tokens
        );
      ...
      if (clones.length > 0) {
        ...
        duplicates.push({
          ...
          clones,
          total_duplicated_lines: clones.reduce(function sum_clone_lines(
            sum,
            c
          ) {
            return sum + c.lines;
  ...
  const total_possible = (functions.length * (functions.length - 1)) / 2;
  const total_skipped =
    comparisons_skipped_by_threshold +
    comparisons_skipped_by_lines +
    comparisons_skipped_by_minhash;
  console.log(
    `[DEDUP] ${functions.length} functions: ${comparisons_made} comparisons made, ` +
      `${total_skipped} skipped (lines:${comparisons_skipped_by_lines}, threshold:${comparisons_skipped_by_threshold}, minhash:${comparisons_skipped_by_minhash}) ` +
      `(${Math.round((1 - comparisons_made / Math.max(1, total_possible)) * 100)}% reduction)`
  );

  // Calculate summary statistics
  const total_duplicated_lines = duplicates.reduce(
    function sum_duplicated_lines(sum, d) {
      return sum + d.total_duplicated_lines;
    },
    0
  );
  ...
  return {
    ...
    summary: {
      ...
      total_clone_count: duplicates.reduce(function count_clones(sum, d) {
        return sum + d.clones.length;
      }, 0),
...
const analyze_dependencies = async (project_id) => {
  ...
  const file_stats = Array.from(files.entries()).map(
    function calculate_file_coupling([filename, stats]) {
      return {
        filename,
        ...
        instability:
          stats.imports + stats.exports > 0
            ? Math.round(
                (stats.imports / (stats.imports + stats.exports)) * 100
              ) / 100
            : 0
      };
    }
  ...
  file_stats.sort(function sort_by_total_coupling(a, b) {
    return (
      b.afferent_coupling +
      b.efferent_coupling -
...
const get_code_metrics = async (project_id) => {
  ...
  const function_metrics = functions.map(function compute_function_metrics(fn) {
  ...
  const most_complex = [...function_metrics]
    .sort(function sort_by_cyclomatic_desc(a, b) {
  ...
  const largest_functions = [...function_metrics]
    .sort(function sort_by_loc_desc(a, b) {
      return b.loc - a.loc;
    })
    .slice(0, 20);
  ...
  const file_metrics = Object.values(by_file).map(
    function calculate_file_avg_complexity(file) {
      return {
        ...file,
        avg_cyclomatic:
  ...
  file_metrics.sort(function sort_by_total_complexity_desc(a, b) {
    return b.total_cyclomatic - a.total_cyclomatic;
  });
...
const analyze_types = async (project_id) => {
      ...
      if (fn.source) {
        ...
        if (return_match) {
          ...
          for (const [type, patterns] of Object.entries(type_patterns)) {
            if (
              patterns.some(function matches_type_pattern(p) {
                return p.test(return_expr);
              })
            ) {
              inferred_type = type;
              break;
  ...
  const dynamic_functions = functions.filter(function is_dynamic_language(f) {
    return dynamic_languages.includes(f.language);
  });
  const type_coverage =
...
const analyze_api_surface = async (project_id) => {
  ...
  for (const fn of functions) {
    const is_private =
      private_patterns.some(function matches_private_pattern(p) {
        return p.test(fn.symbol);
      }) ||
      (fn.source &&
        private_patterns.some(function matches_private_pattern_in_source(p) {
          return p.test(fn.source);
        }));

    const is_public =
      !is_private &&
      (public_patterns.some(function matches_public_pattern(p) {
        return fn.source && p.test(fn.source);
      }) ||
        fn.caller_count > 0);
  ...
  const avg_params =
    public_functions.length > 0
      ? public_functions.reduce(function sum_param_counts(sum, f) {
...
const analyze_documentation = async (project_id) => {
  ...
  for (const fn of functions) {
    ...
    const param_count = fn.parameters
      ? fn.parameters.split(',').filter(function is_non_empty_param(p) {
          return p.trim();
        }).length
      : 0;
  ...
  const file_stats = Object.entries(by_file).map(
    function calculate_file_doc_coverage([filename, stats]) {
      return {
        filename,
        ...stats,
        coverage:
  ...
  file_stats.sort(function sort_by_coverage_asc(a, b) {
    ...
    summary: {
      ...
      coverage_percentage:
        functions.length > 0
          ...
          : 100,
      files_with_poor_coverage: file_stats.filter(
        function has_poor_coverage(f) {
          return f.coverage < 50;
        }
      ).length
    }
  };
  ...
  return {
    ...
    summary: {
      ...
      type_counts: Object.fromEntries(
        Object.entries(by_type).map(function format_type_count([type, defs]) {
          return [type, defs.length];
        })
      ),
      file_count: Object.keys(by_file).length
    }
...
const get_symbol_reference_summary = async (project_id) => {
  ...
  return {
    ...
    summary: {
      ...
      type_counts: Object.fromEntries(
        Object.entries(by_type).map(function format_type_count([type, items]) {
...
const get_class_hierarchy = async (project_id, symbol, options = {}) => {
  ...
  const target_entity = class_entities.find(function find_by_symbol(e) {
  ...
  result.summary = {
    ...
    direct_children: result.descendants.filter(function is_direct_child(d) {
      return d.depth === 1;
    }).length
  };
...
const analyze_class_hierarchy = async (project_id) => {
  ...
  const child_ids = new Set(
    all_relationships.map(function get_child_id(r) {
      return r.child_entity_id;
    })
  );
  const parent_ids = new Set(
    all_relationships
      .filter(function has_parent_entity(r) {
        return r.parent_entity_id;
      })
      .map(function get_parent_id(r) {
        return r.parent_entity_id;
      })
  );

  const root_classes = class_entities.filter(function is_root_class(e) {
    return !all_relationships.some(function is_child_of_entity(r) {
      return r.child_entity_id === e.id;
    });
  });

  // Find leaf classes (no children)
  const leaf_classes = class_entities.filter(function is_leaf_class(e) {
    return !all_relationships.some(function is_parent_of_entity(r) {
      return r.parent_entity_id === e.id;
    });
  });

  // Find abstract classes
  const abstract_classes = class_entities.filter(function is_abstract_class(e) {
  ...
  return {
    ...
    root_classes: root_classes.map(function format_root_class(e) {
      return {
        symbol: e.symbol,
        filename: e.filename,
    ...
    leaf_classes: leaf_classes.map(function format_leaf_class(e) {
      return {
        symbol: e.symbol,
        filename: e.filename,
        start_line: e.start_line,
    ...
    abstract_classes: abstract_classes.map(function format_abstract_class(e) {
      return {
        symbol: e.symbol,
        filename: e.filename,
        start_line: e.start_line
...
export {
  ...
  // Duplication helper functions (exported for testing)
  extract_tokens,
