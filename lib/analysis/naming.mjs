...
const check_convention = (name, type, language) => {
  ...
  const compliant = expected_conventions.some(
    function check_convention_match(exp) {
      return detected_conventions.includes(exp);
    }
  );
...
const analyze_identifier = (name, language) => {
  ...
  const words = name.split(/(?=[A-Z])|_|-/).map(function to_lowercase(w) {
    return w.toLowerCase();
  });
  const abbreviations = words.filter(function is_uncommon_abbreviation(w) {
    return w.length <= 3 && w.length > 0 && !COMMON_ABBREVIATIONS.has(w);
  });
...
const analyze_project_naming = async (project_id) => {
  ...
  results.summary.by_language = Object.fromEntries(
    Object.entries(language_counts).map(function format_language_stats([
      lang,
      counts
    ]) {
      return [
  ...
  results.violations.sort(function sort_by_filename_and_line(a, b) {
