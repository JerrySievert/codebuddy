...
const analyze_identifier_lengths = (source) => {
  ...
  const lengths = identifiers.map(function get_identifier_length(id) {
    return id.length;
  });
  const avg_length =
    lengths.reduce(function sum_lengths(a, b) {
      return a + b;
    }, 0) / lengths.length;

  const too_short = lengths.filter(function is_too_short(l) {
    return l < THRESHOLDS.identifier_length.min_acceptable;
  }).length;
  const too_long = lengths.filter(function is_too_long(l) {
    return l > THRESHOLDS.identifier_length.max_acceptable;
  }).length;

  const good_count = lengths.filter(function is_good_length(l) {
    return (
...
const analyze_line_lengths = (source) => {
  const lines = source.split('\n');
  const lengths = lines
    .map(function get_line_length(l) {
      return l.length;
    })
    .filter(function is_non_empty(l) {
      return l > 0;
    });
  ...
  const over_80 = lengths.filter(function is_over_80(l) {
    return l > 80;
  }).length;
  const over_120 = lengths.filter(function is_over_120(l) {
    return l > 120;
  }).length;
...
const analyze_project_readability = async (project_id) => {
  ...
  function_scores.sort(function sort_by_score_asc(a, b) {
