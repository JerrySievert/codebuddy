...
const calculate_aggregate_complexity = (entities) => {
  ...
  const complexities = entities.map(function calculate_entity_complexity(e) {
    return calculate_complexity(e);
  });

  const total_functions = complexities.length;
  const total_cyclomatic = complexities.reduce(function sum_cyclomatic(sum, c) {
    return sum + c.cyclomatic;
  }, 0);
  const total_loc = complexities.reduce(function sum_loc(sum, c) {
    return sum + c.loc;
  }, 0);
  const total_nesting = complexities.reduce(function sum_nesting_depth(sum, c) {
    return sum + c.nesting_depth;
  }, 0);
  ...
  const max_cyclomatic = complexities.reduce(function find_max_cyclomatic(
    max,
    c
  ) {
    return Math.max(max, c.cyclomatic);
  }, 0);
  const max_nesting_depth = complexities.reduce(function find_max_nesting_depth(
    max,
    c
  ) {
    return Math.max(max, c.nesting_depth);
  }, 0);
