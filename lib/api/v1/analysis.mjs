...
// Code duplication detection - returns cached results from import/refresh
const duplication = {
  method: 'GET',
  ...
  handler: async (request, h) => {
    ...
    // Return cached duplication analysis (computed during import/refresh)
    const cached = await get_project_analysis({
      project_id,
    ...
    // If no cached data, return empty result with message
    return {
      duplicate_groups: [],
      summary: {
        duplicate_group_count: 0,
      ...
      message:
        'Duplication analysis not yet computed. Please refresh the project.'
    };
  }
};
