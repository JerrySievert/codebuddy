...
/**
 * Handler for GET /api/v1/functions/{name} - get entity details (function, class, struct, etc.).
 * @param {Object} request - Hapi request object
 * @param {Object} request.params - Route parameters
 * @param {string} request.params.name - Entity name to retrieve
 ...
 * @returns {Promise<Object[]>} Array of matching entities with complexity metrics, or 404
 */
const retrieve_handler = async (request, h) => {
  ...
  // If type is specified, use it; otherwise search all entity types
  ...
  if (results.length === 0) {
    return h.response({ error: `Entity '${name}' not found` }).code(404);
  }
