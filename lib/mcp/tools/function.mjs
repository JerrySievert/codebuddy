 ...
 */
export const function_list_handler = async ({ project_name, filename }) => {
 ...
 */
export const function_retrieve_handler = async ({
  name,
  project_name,
  filename
}) => {
 ...
 */
export const function_callers_handler = async ({ name, project_name }) => {
  let project_id;
  if (project_name !== undefined) {
    const projects = await get_project_by_name({ name: project_name });
    if (projects.length !== 0) {
 ...
 */
export const function_callees_handler = async ({ name, project_name }) => {
  let project_id;
  if (project_name !== undefined) {
    const projects = await get_project_by_name({ name: project_name });
    if (projects.length !== 0) {
 ...
 */
export const function_complexity_handler = async ({
  name,
  project_name,
  filename
}) => {
 ...
 */
export const function_controlflow_handler = async ({
  name,
  project_name,
  filename
}) => {
...
Use depth parameter to control how far to traverse (default 10, 0 for unlimited).`,
    schema: {
      ...
      project_name: z.string().optional().describe('Project name to filter by'),
      filename: z
        .string()
        .optional()
        .describe(
...
- 50+: Very high risk, should be refactored`,
    schema: {
      ...
      project_name: z.string().optional().describe('The name of the project (use project_list to see available projects)'),
...
Returns nodes and edges that can be used to render a flowchart visualization.`,
    schema: {
      ...
      project_name: z.string().describe('The name of the project (required, use project_list to see available projects)'),
      filename: z.string().optional().describe('Filename to disambiguate')
    },
