...
const batch_insert_inheritance = async (relationships) => {
  ...
  const valid_rels = relationships.filter(
    (r) =>
      r.child_entity_id != null &&
      r.parent_symbol != null &&
      r.relationship_type != null
  );

  if (valid_rels.length === 0) return [];

  const child_entity_ids = valid_rels.map((r) => r.child_entity_id);
  const parent_entity_ids = valid_rels.map((r) => r.parent_entity_id || null);
  const parent_symbols = valid_rels.map((r) => r.parent_symbol);
  const relationship_types = valid_rels.map((r) => r.relationship_type);

  return await query`
    INSERT INTO inheritance (child_entity_id, parent_entity_id, parent_symbol, relationship_type)
    SELECT * FROM UNNEST(
      ${child_entity_ids}::int[],
...
const get_inheritance_stats = async (project_id) => {
  ...
  const root_classes = await query`
  ...
  const deepest_hierarchy = await query`
  ...
  return {
    by_type: stats,
    root_class_count: parseInt(root_classes[0]?.count || 0),
    max_hierarchy_depth: parseInt(deepest_hierarchy[0]?.max_depth || 0)
  };
};
