...
const build_call_graph = async ({
  ...
  // This avoids:
  // 1. Two separate queries (callee + caller)
  // 2. Duplicating entity data in the recursive CTE results
  // 3. Fetching large 'source' column multiple times per node
  const graph_data = await query`
    ...
    all_edges AS (
      SELECT DISTINCT caller, callee, line FROM callee_graph
      UNION
      SELECT DISTINCT caller, callee, line FROM caller_graph
    ),
    -- Collect unique node IDs from edges
    node_ids AS (
      SELECT caller AS id FROM all_edges
      UNION
      SELECT callee AS id FROM all_edges
      UNION
      SELECT ${root_id} AS id
    )
    ...
    SELECT
      'edge' AS row_type,
      ae.caller AS edge_from,
      ae.callee AS edge_to,
      ae.line AS edge_line,
      NULL::int AS node_id,
      NULL::text AS node_symbol,
      NULL::text AS node_filename,
      NULL::int AS node_start_line,
