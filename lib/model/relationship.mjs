...
import { get_entity } from './entity.mjs';
...
/**
 * Build a reverse call graph (callers only) starting from an entity.
 * Returns nodes and edges suitable for graph visualization.
 * Only traverses upstream (callers of callers), not downstream.
 * @param {Object} params - Query parameters
 * @param {string} params.symbol - Symbol name of the root function
 * @param {number} params.project_id - The project ID
 * @param {string} [params.filename] - Optional filename to disambiguate same-named functions
 ...
 * @returns {Promise<Object>} Graph data with { root, nodes, edges }
 */
const build_reverse_call_graph = async ({
  symbol,
  project_id,
  filename,
  max_depth = 5
  ...
  // Query that only fetches caller relationships (upstream direction)
  const graph_data = await query`
    WITH RECURSIVE
    -- Traverse callers only (functions that call root and ancestors)
    caller_graph AS (
      SELECT r.caller, r.callee, r.line, 1 as depth
      FROM relationship r
      WHERE r.callee = ${root_id}
