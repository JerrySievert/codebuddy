...
const batch_insert_symbol_references = async (references) => {
  ...
  const validRefs = references.filter(
    (r) => r.project_id != null && r.symbol != null && r.filename != null && r.line != null && r.symbol_type != null
  );

  if (validRefs.length === 0) return [];

  const projectIds = validRefs.map((r) => r.project_id);
  const symbols = validRefs.map((r) => r.symbol);
  const symbolTypes = validRefs.map((r) => r.symbol_type);
  const definitionEntityIds = validRefs.map((r) => r.definition_entity_id || null);
  const filenames = validRefs.map((r) => r.filename);
  const lines = validRefs.map((r) => r.line);
  const columnStarts = validRefs.map((r) => r.column_start || null);
  const columnEnds = validRefs.map((r) => r.column_end || null);
  const contexts = validRefs.map((r) => r.context || null);
  // Convert booleans to strings for PostgreSQL array casting
  const isDefinitions = validRefs.map((r) => (r.is_definition ? 't' : 'f'));
  const isWrites = validRefs.map((r) => (r.is_write ? 't' : 'f'));
 ...
 */
const get_symbol_references = async ({ project_id, symbol, filename, is_definition }) => {
  return await query`
    SELECT
      sr.id,
      sr.project_id,
 ...
 */
const get_definition_for_symbol = async ({ project_id, symbol, filename, line }) => {
