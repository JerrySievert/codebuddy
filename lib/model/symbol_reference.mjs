 ...
 * NOTE: We keep idx_symbol_reference_project_id because it's critical for DELETE operations
 * and dropping/recreating it on large tables takes too long.
 * @returns {Promise<void>}
 */
const disable_symbol_reference_indexes = async () => {
  // Drop indexes (they will be recreated after bulk load)
  // NOTE: Do NOT drop idx_symbol_reference_project_id - it's needed for fast deletes
...
const rebuild_symbol_reference_indexes = async () => {
  ...
  // Use IF NOT EXISTS for project_id index since we don't drop it
  await Promise.all([
    query`CREATE INDEX IF NOT EXISTS idx_symbol_reference_project_id ON symbol_reference(project_id)`,
    query`CREATE INDEX IF NOT EXISTS idx_symbol_reference_project_symbol ON symbol_reference(project_id, symbol)`,
    query`CREATE INDEX IF NOT EXISTS idx_symbol_reference_filename ON symbol_reference(filename)`,
    query`CREATE INDEX IF NOT EXISTS idx_symbol_reference_definition ON symbol_reference(definition_entity_id)`
  ]);
};
