...
const batch_insert_references = async (references) => {
  ...
  const validReferences = references.filter(
    (r) => r.entity_id != null && r.filename != null && r.line != null && r.reference_type != null
  );

  if (validReferences.length === 0) return [];

  const entityIds = validReferences.map((r) => r.entity_id);
  const filenames = validReferences.map((r) => r.filename);
  const lines = validReferences.map((r) => r.line);
  const referenceTypes = validReferences.map((r) => r.reference_type);
  const contexts = validReferences.map((r) => r.context || null);
 ...
 */
const get_references_by_symbol = async ({ symbol, project_id, reference_type }) => {
  return await query`
    SELECT
      r.id,
      r.entity_id,
