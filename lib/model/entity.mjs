...
/**
 * Batch insert entities without conflict handling (for fresh imports).
 * Much faster than batch_insert_or_update_entities for large imports
 * because it skips the expensive unique constraint checks.
 * Use after clear_entities_for_project for best performance.
 * @param {Object[]} entities - Array of entity objects to insert
 * @returns {Promise<Object[]>} Array of inserted entity records with IDs
 */
const batch_insert_entities_fast = async (entities) => {
  if (entities.length === 0) return [];

  // Deduplicate entities by their unique key (project_id, language, symbol, type, filename)
  // Keep the last occurrence (in case of duplicates within the batch)
  ...
  // Simple INSERT without ON CONFLICT - much faster for fresh imports
  const ret = await query`
    ...
    RETURNING id, symbol, filename, start_line, end_line
  `;
...
/**
 * Drop the partial index for a project if it exists.
 * Called when a project is deleted.
 * @param {number} project_id - The project ID
 * @returns {Promise<boolean>} True if index was dropped, false if didn't exist
 */
const drop_partial_index_for_project = async (project_id) => {
  const index_name = `idx_entity_project_${project_id}_type_symbol`;
  ...
  await query.unsafe(`DROP INDEX IF EXISTS ${index_name}`);
  return true;
};
...
export {
  ...
  drop_partial_index_for_project,
