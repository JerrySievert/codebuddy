 ...
 * @returns {Promise<void>}
 */
const delete_project = async (id, on_progress = null) => {
  ...
  report(10, 'Deleting relationships...');
  await query`
    DELETE FROM relationship
     WHERE caller IN (SELECT id FROM entity WHERE project_id = ${id})
        OR callee IN (SELECT id FROM entity WHERE project_id = ${id})
  `;

  // Delete symbol references
  report(25, 'Deleting symbol references...');
  await query`
    DELETE FROM symbol_reference WHERE project_id = ${id}
  `;

  // Delete inheritance records
  report(40, 'Deleting inheritance records...');
  await query`
    DELETE FROM inheritance
     WHERE child_entity_id IN (SELECT id FROM entity WHERE project_id = ${id})
        OR parent_entity_id IN (SELECT id FROM entity WHERE project_id = ${id})
  `;

  // Delete cached project analysis
  report(55, 'Deleting cached analysis...');
  await query`
    DELETE FROM project_analysis WHERE project_id = ${id}
  `;

  // Delete entities
  report(65, 'Deleting entities...');
  await query`
    DELETE FROM entity WHERE project_id = ${id}
  `;

  // Delete source code
  report(80, 'Deleting source code...');
  await query`
    DELETE FROM sourcecode WHERE project_id = ${id}
  `;

  // Delete the project
  report(90, 'Deleting project record...');
  await query`
    DELETE FROM project WHERE id = ${id}
  `;

  // Refresh the stats view
  report(95, 'Refreshing stats...');
