...
const printTree = (nodes, prop, indent = '') => {
  for (let i = 0; i < nodes.length; i++) {
    ...
    if (node[prop] && node[prop].length > 0) {
      printTree(node[prop], prop, indent + childIndent);
    }
  }
};

const hierarchy_tree = async ({ project, symbol, direction, 'max-depth': maxDepth }) => {
  const project_id = await getProjectId(project);
  const result = await get_class_hierarchy(project_id, symbol, {
    direction: direction || 'both',
    max_depth: maxDepth ? parseInt(maxDepth) : 10
  });
  ...
  console.log(`Summary: ${result.summary.ancestor_count} ancestors, ${result.summary.descendant_count} descendants`);
  console.log(`Direct Parents: ${result.summary.direct_parents}, Direct Children: ${result.summary.direct_children}\n`);

  if (result.ancestors && result.ancestors.length > 0) {
    console.log('Ancestors (parents and their parents):');
    printTree(result.ancestors, 'ancestors');
    console.log();
  }

  if (result.descendants && result.descendants.length > 0) {
    console.log('Descendants (children and their children):');
    printTree(result.descendants, 'descendants');
    console.log();
  }
...
const hierarchy_implementations = async ({ project, symbol }) => {
  ...
  console.log(`Extends: ${result.summary.extends_count}, Implements: ${result.summary.implements_count}, Embeds: ${result.summary.embeds_count}\n`);
  ...
  for (const [type, impls] of Object.entries(result.by_relationship_type)) {
    ...
    for (const impl of impls) {
      ...
      if (impl.child_filename) {
        console.log(`    ${impl.child_filename}:${impl.child_start_line || '?'}`);
      }
    }
    console.log();
  }
...
const hierarchy_analysis = async ({ project }) => {
  ...
  if (result.deepest_inheritance) {
    ...
    console.log(`  ${result.deepest_inheritance.symbol} (depth: ${result.deepest_inheritance.depth})`);
  ...
  if (result.root_classes.length > 0) {
    ...
    for (const cls of result.root_classes.slice(0, 10)) {
      console.log(`  ${cls.symbol} (${cls.type}) - ${cls.filename}:${cls.start_line}`);
    }
