...
const analysis_naming = async ({ project }) => {
  ...
  if (result.convention_distribution && Object.keys(result.convention_distribution).length > 0) {
    console.log('Convention Distribution:');
    for (const [conv, count] of Object.entries(result.convention_distribution)) {
      const percent = Math.round((count / result.summary.total_identifiers) * 100);
  ...
  // By language
  if (result.summary.by_language && Object.keys(result.summary.by_language).length > 0) {
    console.log('By Language:');
    for (const [lang, stats] of Object.entries(result.summary.by_language)) {
      console.log(`  ${lang}: ${stats.compliant}/${stats.total} compliant (${stats.compliance_rate}%)`);
    }
    console.log();
  }
  ...
  if (result.violations && result.violations.length > 0) {
    console.log(`Naming Convention Violations (${result.violations.length}${result.violations_truncated ? '+' : ''}):\n`);
    for (const v of result.violations.slice(0, 20)) {
      console.log(`  * ${v.symbol} (${v.type})`);
      console.log(`      ${v.filename}:${v.line}`);
      console.log(`      Expected: ${v.expected.join(' or ')}, Found: ${v.detected.join(', ')}`);
    }
  ...
  if (result.issues && result.issues.length > 0) {
    console.log(`Naming Issues (${result.issues.length}${result.issues_truncated ? '+' : ''}):\n`);
...
const analysis_readability = async ({ project }) => {
  ...
  console.log(`  Excellent (85+): ${result.summary.score_distribution.excellent}`);
  console.log(`  Good (70-84): ${result.summary.score_distribution.good}`);
  console.log(`  Acceptable (55-69): ${result.summary.score_distribution.acceptable}`);
  console.log(`  Poor (40-54): ${result.summary.score_distribution.poor}`);
  console.log(`  Very Poor (<40): ${result.summary.score_distribution.very_poor}`);
  console.log();

  // Metrics
  console.log('Aggregate Metrics:');
  console.log(`  Avg Function Length: ${result.metrics.avg_function_length} lines`);
  ...
  console.log(`  Total Complex Booleans: ${result.metrics.total_complex_booleans}`);
  console.log();
  ...
  if (result.summary.by_language && Object.keys(result.summary.by_language).length > 0) {
    console.log('By Language:');
    for (const [lang, stats] of Object.entries(result.summary.by_language)) {
      console.log(`  ${lang}: ${stats.count} functions, avg score ${stats.avg_score}`);
    }
    console.log();
  }
  ...
  if (result.worst_functions && result.worst_functions.length > 0) {
    ...
    for (const f of result.worst_functions.slice(0, 10)) {
      ...
      if (f.issues) {
        const issues = [];
        if (f.issues.nesting_depth > 4) issues.push(`nesting: ${f.issues.nesting_depth}`);
        if (f.issues.magic_numbers > 5) issues.push(`magic numbers: ${f.issues.magic_numbers}`);
        if (f.issues.complex_booleans > 0) issues.push(`complex booleans: ${f.issues.complex_booleans}`);
        if (f.issues.max_line_length > 120) issues.push(`max line: ${f.issues.max_line_length}`);
        if (issues.length > 0) {
...
const analysis_patterns = async ({ project }) => {
  ...
  console.log(`  Functions with Patterns: ${result.summary.functions_with_patterns}`);
  console.log(`  Total Patterns Detected: ${result.summary.total_patterns_detected}`);
  console.log();

  // By category
  console.log('Patterns by Category:');
  console.log(`  Creational: ${result.summary.patterns_by_category.creational}`);
  console.log(`  Structural: ${result.summary.patterns_by_category.structural}`);
  console.log(`  Behavioral: ${result.summary.patterns_by_category.behavioral}`);
  console.log(`  Anti-patterns: ${result.summary.patterns_by_category['anti-pattern']}`);
  console.log();

  // Error handling summary
  if (result.summary.error_handling) {
    console.log('Error Handling:');
    console.log(`  Try/Catch Blocks: ${result.summary.error_handling.total_try_catch}`);
    console.log(`  Empty Catch Blocks: ${result.summary.error_handling.total_empty_catch}`);
    console.log(`  Finally/Defer Blocks: ${result.summary.error_handling.total_finally}`);
    console.log(`  Functions with Error Handling: ${result.summary.error_handling.functions_with_error_handling}`);
    console.log();
  }
  ...
  if (Object.keys(result.patterns).length > 0) {
    ...
    for (const [id, data] of Object.entries(result.patterns)) {
      console.log(`  * ${data.name} (${data.category}): ${data.count} occurrences`);
  ...
  if (result.error_handling_issues && result.error_handling_issues.length > 0) {
    console.log(`Error Handling Issues (${result.error_handling_issues.length}):\n`);
    for (const issue of result.error_handling_issues.slice(0, 10)) {
      console.log(`  [${issue.severity}] ${issue.symbol} - ${issue.filename}:${issue.line}`);
      console.log(`    ${issue.message}`);
    }
    console.log();
  }
...
const analysis_tests = async ({ project }) => {
  ...
  console.log(`  Avg Assertions per Test: ${result.summary.avg_assertions_per_test}`);
  ...
  if (result.summary.by_language && Object.keys(result.summary.by_language).length > 0) {
    console.log('By Language:');
    for (const [lang, stats] of Object.entries(result.summary.by_language)) {
      console.log(`  ${lang}: ${stats.test_functions}/${stats.total_functions} test functions (${stats.test_ratio}%)`);
    }
    console.log();
  }
  ...
  if (result.test_functions && result.test_functions.length > 0) {
    ...
    for (const f of result.test_functions.slice(0, 10)) {
      console.log(`  * ${f.symbol} - ${f.assertions} assertions, ${f.mocks} mocks`);
  ...
  if (result.coverage_gaps && result.coverage_gaps.length > 0) {
    console.log(`Potential Coverage Gaps (${result.coverage_gaps.length}${result.coverage_gaps_truncated ? '+' : ''}):\n`);
  ...
  if (result.naming_issues && result.naming_issues.length > 0) {
    console.log(`Test Naming Issues (${result.naming_issues.length}${result.naming_issues_truncated ? '+' : ''}):\n`);
