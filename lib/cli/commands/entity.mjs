...
const entity_references = async ({ name, project, type }) => {
  ...
  for (const [refType, refs] of Object.entries(grouped)) {
    console.log(`  ${refType.charAt(0).toUpperCase() + refType.slice(1)}s (${refs.length}):`);
...
const entity = {
  ...
  command_arguments: {
    ...
    references: {
      ...
      type: {
        type: 'string',
        description: 'Reference type (variable, parameter, field, typedef, macro)'
      }
    }
  }
};
