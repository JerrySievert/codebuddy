...
Gives access to entities (functions, classes, structs), allowing for listing, searching, and retrieving members.
  ...
  * references - Lists all code locations where a struct/class is referenced
`;
...
const references_help = `usage: cb entity references --name=[name] --project=[project] [--type=<type>]

List all code locations where a struct or class is referenced.

Arguments:

  * --name=[name] - Name of the struct or class (required)
  * --project=[project] - Name of the project (required)
  * --type=[type] - Filter by reference type (variable, parameter, field, typedef, macro)
`;
...
const entity_list = async ({ project, filename, type }) => {
  ...
  for (const [entity_type, entities] of Object.entries(grouped)) {
    console.log(
      `\n${entity_type.charAt(0).toUpperCase() + entity_type.slice(1)}s:\n`
    );
...
const entity_members = async ({ id }) => {
  ...
  console.log(
    `File: ${entity.filename}:${entity.start_line}-${entity.end_line}\n`
  );
  ...
  for (const member of members) {
    ...
    const return_type = member.return_type ? `: ${member.return_type}` : '';
    console.log(
      `  [${member.id}] ${member.symbol}${params}${return_type} - line ${member.start_line}`
    );
  }
};

const entity_references = async ({ name, project, type }) => {
  const projects = await get_project_by_name({ name: project });
  ...
  // Group by reference type for better display
  const grouped = {};
  for (const ref of references) {
    if (!grouped[ref.reference_type]) {
      grouped[ref.reference_type] = [];
    }
  ...
  for (const [ref_type, refs] of Object.entries(grouped)) {
    console.log(
      `  ${ref_type.charAt(0).toUpperCase() + ref_type.slice(1)}s (${refs.length}):`
    );
    for (const ref of refs) {
      console.log(`    ${ref.filename}:${ref.line}`);
      if (ref.context) {
  ...
  console.log(`Total: ${references.length} references`);
};
...
const entity = {
  ...
  command_arguments: {
    ...
    references: {
      name: {
        type: 'string',
        description: 'Name of the struct or class',
        required: true
      },
      project: {
        type: 'string',
      ...
      type: {
        type: 'string',
        description:
          'Reference type (variable, parameter, field, typedef, macro)'
      }
    }
  }
};
