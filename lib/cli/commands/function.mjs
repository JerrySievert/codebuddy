...
const print_call_graph_node = (
  node,
  prefix = '',
  is_last = true,
  direction = 'root'
) => {
  const connector = prefix === '' ? '' : is_last ? '└── ' : '├── ';
  const dir_indicator =
    direction === 'caller' ? '← ' : direction === 'callee' ? '→ ' : '';

  console.log(
    `${prefix}${connector}${dir_indicator}${node.symbol} (${node.filename}:${node.start_line})`
  );

  const new_prefix = prefix + (prefix === '' ? '' : is_last ? '    ' : '│   ');

  // Print callers
  if (node.callers && node.callers.length > 0) {
    for (let i = 0; i < node.callers.length; i++) {
      print_call_graph_node(
        node.callers[i],
        new_prefix,
        i === node.callers.length - 1 &&
...
const print_tree_node = (node, prefix = '', is_last = true) => {
  const connector = prefix === '' ? '' : is_last ? '└── ' : '├── ';
  const params = node.parameters || '';
  console.log(
    `${prefix}${connector}${node.symbol}${params} (${node.filename}:${node.start_line})`
  );
  ...
  for (let i = 0; i < children.length; i++) {
    print_tree_node(children[i], new_prefix, i === children.length - 1);
  }
};
...
const function_call_graph = async ({ name, project, depth = 3, filename }) => {
  ...
  print_call_graph_node(graph.root);
...
const function_control_flow = async ({ name, project, filename }) => {
  ...
  for (const node of cfg.nodes) {
    ...
    if (node.code) {
      const code_preview =
        node.code.length > 60 ? node.code.substring(0, 60) + '...' : node.code;
      console.log(`       Code: ${code_preview.replace(/\n/g, ' ')}`);
    }
  }
