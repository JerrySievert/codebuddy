...
const reference_find = async ({
  project,
  symbol,
  filename,
  'definitions-only': definitions_only
}) => {
  ...
  console.log(
    `Total: ${result.summary.total_references} references in ${result.summary.file_count} files`
  );
  console.log(
    `Definitions: ${result.summary.definition_count}, Reads: ${result.summary.read_count}, Writes: ${result.summary.write_count}\n`
  );

  if (result.primary_definition) {
    console.log(
      `Primary Definition: ${result.primary_definition.filename}:${result.primary_definition.line}\n`
    );
  }
  ...
  for (const [file, refs] of Object.entries(result.by_file)) {
    ...
    for (const ref of refs) {
      ...
      const flag_str = flags.length > 0 ? ` [${flags.join(', ')}]` : '';
      console.log(
        `  ${ref.line}:${ref.column_start} - ${ref.symbol_type}${flag_str}`
      );
      if (ref.context) {
...
const reference_definition = async ({ project, symbol, filename, line }) => {
  const project_id = await get_project_id(project);
  const result = await go_to_definition(project_id, symbol, {
    filename,
...
const reference_list = async ({ project, type }) => {
  ...
  console.log(
    `Total: ${result.summary.total_definitions} definitions in ${result.summary.file_count} files\n`
  );
  ...
  for (const [file, defs] of Object.entries(result.by_file)) {
    if (shown >= max_show) {
      console.log(
        `... and ${result.summary.total_definitions - shown} more definitions`
      );
      break;
    }
    console.log(`${file}:`);
...
const reference_summary = async ({ project }) => {
  ...
  for (const item of result.top_symbols.slice(0, 20)) {
    console.log(
      `  ${item.symbol} (${item.symbol_type}): ${item.reference_count} refs, ${item.definition_count} defs`
    );
  }
};
...
const reference_at = async ({ project, filename, line, column }) => {
  ...
  for (const sym of result) {
    ...
    if (sym.definition_filename) {
      console.log(
        `  Defined at: ${sym.definition_filename}:${sym.definition_line}`
      );
    }
    if (sym.context) {
