    ...
    resource: [
      {
        ...
        pattern: /\busing\s*\(\s*var\s+\w|\busing\s*\(\s*\w+\s+\w|\busing\s+var\s+/,
        safe: true,
...
const analyze_function_resources = (fn) => {
  ...
  for (const [category, patternList] of Object.entries(patterns)) {
    for (const p of patternList || []) {
      ...
      const hasAcquire = p.acquire && p.acquire.test(source);
      const hasRelease = p.release && p.release.test(source);
      const hasWithStatement = p.with_statement && p.with_statement.test(source);
      ...
      if (hasAcquire) {
        ...
        if (!hasRelease && !hasWithStatement) {
          // Check if defer is used (Go) or if it's a safe context
          const hasDefer = language === 'go' && /\bdefer\s+/.test(source);
          const hasTryWithResources = language === 'java' && /\btry\s*\([^)]*\)\s*\{/.test(source);
          const hasUsing = language === 'csharp' && /\busing\s*\(/.test(source);
          const hasWith = language === 'python' && /\bwith\s+/.test(source);

          if (!hasDefer && !hasTryWithResources && !hasUsing && !hasWith) {
  ...
  if (language === 'cpp') {
    ...
    if (/\bnew\s+\w/.test(source) && !/std::(unique_ptr|shared_ptr|make_unique|make_shared)/.test(source)) {
      findings.warnings.push({
        id: 'raw_new_without_smart_ptr',
        description: 'Using raw new without smart pointers - consider using std::make_unique or std::make_shared',
        severity: 'suggestion',
        function_id: fn.id,
        symbol: fn.symbol,
        filename: fn.filename,
  ...
  if (language === 'c') {
    ...
    if (/\bmalloc\s*\(/.test(source) && !/if\s*\(\s*\w+\s*(==|!=)\s*(NULL|0|nullptr)/.test(source)) {
      findings.warnings.push({
...
const analyze_project_resources = async (project_id) => {
  ...
  for (const fn of functions) {
    ...
    if (hasResources || findings.potential_leaks.length > 0) {
      ...
      results.by_file[fn.filename].acquisition_count += findings.acquisitions.length;
      results.by_file[fn.filename].release_count += findings.releases.length;
      results.by_file[fn.filename].safe_pattern_count += findings.safe_patterns.length;
      results.by_file[fn.filename].potential_leak_count += findings.potential_leaks.length;
      ...
      results.by_language[fn.language].acquisition_count += findings.acquisitions.length;
      results.by_language[fn.language].release_count += findings.releases.length;
      results.by_language[fn.language].safe_pattern_count += findings.safe_patterns.length;
      results.by_language[fn.language].potential_leak_count += findings.potential_leaks.length;
  ...
  results.summary = {
    ...
    resource_percentage: functions.length > 0
      ? Math.round((results.functions_with_resources.length / functions.length) * 100 * 10) / 10
      : 0,
  ...
  results.acquisitions_grouped = groupByName(results.acquisitions);
  results.releases_grouped = groupByName(results.releases);
  results.safe_patterns_grouped = groupByName(results.safe_patterns);
  results.potential_leaks_grouped = groupByName(results.potential_leaks);
