 ...
 * @param {string} source - The source code to parse
 * @param {string} filename - Filename used to determine language
 * @param {string[]} known_types - Array of known struct/class type names to match
 * @returns {Object[]} Array of reference objects with type_name, reference_type, line, context
 */
const get_type_references_from_source = (
  source,
  filename,
  known_types = []
) => {
  ...
  const reference_type_set = new Set(config.referenceTypes);
  const source_lines = source.split('\n');

  // Track seen macro references to avoid O(n) duplicate check
  const seen_macro_refs = new Set();
  ...
  while (true) {
    if (!visited_children) {
      ...
      // Check if this is a reference-related node type (O(1) Set lookup)
      if (reference_type_set.has(node_type)) {
        // Extract type identifiers from this node
        const type_ids = extract_type_identifiers(node);

        for (const type_name of type_ids) {
          // Only track references to known struct/class types
          if (known_type_set.size === 0 || known_type_set.has(type_name)) {
      ...
      // Only process if we have known types to search for
      if (
        known_type_set.size > 0 &&
        (node_type === 'preproc_def' || node_type === 'preproc_function_def')
      ) {
        const content = node.text;
        const line = node.startPosition.row + 1;

        // Only check types that could plausibly be in this macro (simple length check)
        for (const type_name of known_types) {
          // Check if the macro content mentions any known type
          if (content.includes(type_name)) {
            // O(1) duplicate check using Set
            const ref_key = `${type_name}:${line}`;
            if (!seen_macro_refs.has(ref_key)) {
              seen_macro_refs.add(ref_key);
              const context =
                source_lines[node.startPosition.row]?.trim() || '';
