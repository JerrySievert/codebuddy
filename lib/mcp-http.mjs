...
import { is_mcp_disabled } from './config.mjs';
    ...
    res.end(
      JSON.stringify({
        ...
        error: {
          code: -32603,
          message:
            'MCP is disabled on this server. Start the server without --disable-mcp to enable MCP.'
        },
        id: null
      })
    );
  ...
  try {
    ...
    if (session_id && transports[session_id]) {
      ...
      transport = new StreamableHTTPServerTransport({
        sessionIdGenerator: function generate_session_id() {
          return randomUUID();
        },
        onsessioninitialized: function handle_session_initialized(sid) {
      ...
      transport.onclose = function handle_transport_close() {
