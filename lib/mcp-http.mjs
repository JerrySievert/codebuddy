...
const create_mcp_server = () => {
  ...
  server.tool(
    ...
    `Lists all entities (functions, classes, structs, etc.) in a project, optionally filtered by filename or type.`,
    {
      ...
      type: z
        .string()
        .optional()
        .describe('Filter by entity type (function, class, struct)')
    },
    ...
    async ({ entity_id }) => {
      try {
        ...
        if (entity.type !== 'class' && entity.type !== 'struct') {
          throw new Error(
            `Entity is not a class or struct (type: ${entity.type})`
          );
        }
  ...
  server.tool(
    ...
    `Builds a bidirectional call graph centered on a function, showing both callers and callees.`,
    {
      ...
      depth: z
        ...
        .optional()
        .default(3)
        .describe('Maximum depth to traverse (max 5)'),
      filename: z
        .string()
        .optional()
        .describe(
  ...
  server.tool(
    ...
    async ({ name }) => {
      try {
        ...
        return {
          content: [
            { type: 'text', text: `Project '${name}' deleted successfully` }
          ]
        };
      } catch (err) {
        return {
  ...
  server.tool(
    ...
    async ({ project, symbol, filename, line }) => {
      try {
        ...
        const result = await go_to_definition(projects[0].id, symbol, {
          filename,
          line
        });
        return {
  ...
  server.tool(
    ...
    async ({ project, type }) => {
      try {
        ...
        const result = await list_definitions(projects[0].id, {
          symbol_type: type
        });
        return {
          content: [{ type: 'text', text: JSON.stringify(result) }]
  ...
  server.tool(
    ...
    async ({ project, symbol, direction, max_depth }) => {
      try {
        ...
        const result = await get_class_hierarchy(projects[0].id, symbol, {
          direction,
          max_depth
        });
        return {
  ...
  server.tool(
    'analysis_patterns',
    `Detects design patterns and anti-patterns in a codebase. Identifies creational patterns (Singleton, Factory, Builder), structural patterns (Adapter, Decorator, Facade, Proxy), behavioral patterns (Observer, Strategy, Command, State, Iterator), and anti-patterns (Callback Hell). Also analyzes error handling patterns.`,
    {
      project: z.string().describe('Project name')
    },
    async ({ project }) => {
      try {
        ...
        const result = await analyze_project_design_patterns(projects[0].id);
        return {
          content: [{ type: 'text', text: JSON.stringify(result) }]
        };
      } catch (err) {
  ...
  server.tool(
    'analysis_tests',
    `Analyzes test code and coverage patterns. Detects test files, calculates test-to-code ratio, measures assertion density, tracks mock/stub usage, checks test naming conventions, and identifies potential coverage gaps.`,
    {
      project: z.string().describe('Project name')
    },
    async ({ project }) => {
      try {
        ...
        const result = await analyze_project_test_coverage(projects[0].id);
        return {
          content: [{ type: 'text', text: JSON.stringify(result) }]
        };
      } catch (err) {
...
export { mcp_routes, create_mcp_server };
