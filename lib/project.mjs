...
/**
 * Check if content appears to be binary (contains null bytes or other binary indicators).
 * @param {string} content - The file content to check
 * @returns {boolean} True if the content appears to be binary
 */
const is_binary_content = (content) => {
  // Check for null bytes (common in binary files)
  if (content.includes('\x00')) {
    return true;
  }
  // Check for high ratio of non-printable characters (excluding common whitespace)
  const sample = content.slice(0, 8000); // Check first 8KB
  let non_printable = 0;
  ...
  return sample.length > 0 && non_printable / sample.length > 0.1;
};

/**
 * Import non-code files (markdown, text, config, etc.) into the sourcecode table.
 * These files are stored for viewing but not parsed for entities/relationships.
 * Binary files are automatically skipped.
 * @param {Object} context - Phase context
 * @returns {Promise<void>}
 * @private
 */
const import_other_files = async (context) => {
  const { project_id, other_files, working_path, on_progress } = context;

  if (other_files.length === 0) {
    return;
  }

  console.log(`Importing ${other_files.length} non-code files...`);
  if (on_progress) {
    on_progress(68, `Importing ${other_files.length} non-code files...`);
  }
  ...
  for (const absolute_filename of other_files) {
    try {
      const content = await import_file(absolute_filename);
      ...
      const relative_filename = make_relative_path(
        absolute_filename,
        working_path
      );
      ...
      if (batch.length >= BATCH_SIZE) {
        await batch_insert_or_update_sourcecode(batch);
        imported_count += batch.length;
        batch = [];

        if (on_progress) {
          const progress =
            68 + Math.floor((imported_count / other_files.length) * 4);
          on_progress(
            progress,
            `Imported ${imported_count}/${other_files.length} non-code files`
          );
        }
      }
    } catch (error) {
  ...
  console.log(
    `  Imported ${imported_count} non-code files (skipped ${skipped_count} binary/unreadable files)`
  );
};
...
const create_or_update_project = async ({
  ...
  // Discover all files
  const all_files = await get_all_filenames(working_path);

  if (all_files.length === 0) {
    throw new Error('No files found.');
  }

  // Separate code files (for parsing) from other files (just store content)
  const code_extensions = new Set(types.map((t) => `.${t}`));
  const files = []; // Code files to parse
  const other_files = []; // Non-code files to just store
  ...
  // Memory-saving thresholds (based on total files, not just code files)
  const total_file_count = files.length + other_files.length;
  const LARGE_REPO_THRESHOLD = 300;
  const VERY_LARGE_REPO_THRESHOLD = 1000;
  const INDEX_DROP_THRESHOLD = 5000;
  const use_memory_saving_mode = total_file_count > LARGE_REPO_THRESHOLD;
  const should_drop_indexes = total_file_count > INDEX_DROP_THRESHOLD;

  console.log(
    `Found ${files.length} code files and ${other_files.length} other files to process`
  );
  if (total_file_count > VERY_LARGE_REPO_THRESHOLD) {
    console.log(
  ...
  // Import non-code files (markdown, text, config files, etc.)
  await import_other_files(context);
