...
const get_class_name = (entity, language) => {
  ...
  const content = entity.content;
  const classMatch = content.match(/(?:class|struct|interface|enum|type)\s+(\w+)/);
  if (classMatch) {
    return classMatch[1];
  }
...
const create_project = async ({ name, path }) => {
  ...
  try {
    ...
    if (is_git_url(path)) {
      ...
      console.log(`[PROJECT] Checking if path exists: "${localPath}"`);
      console.log(`[PROJECT] existsSync result: ${existsSync(localPath)}`);
...
const create_or_update_project = async ({
  ...
  // Phase 4: Extract and store type references
  console.log('Phase 4: Extracting type references...');
  const refStart = Date.now();

  // Clear existing references before repopulating
  await clear_references_for_project(project[0]);

  // Get known struct/class type names for reference matching
  const knownTypeNames = Object.values(known_entities)
    .filter((e) => e.type === 'struct' || e.type === 'class')
    .map((e) => e.symbol);

  // Build a map of type name to entity id for fast lookup
  const typeNameToEntity = new Map();
  for (const entity of Object.values(known_entities)) {
    if (entity.type === 'struct' || entity.type === 'class') {
      typeNameToEntity.set(entity.symbol, entity);
    }
  }
  ...
  if (knownTypeNames.length > 0) {
    ...
    // Re-parse files to extract type references
    for (let i = 0; i < files.length; i += PARSE_BATCH_SIZE) {
      const batch = files.slice(i, i + PARSE_BATCH_SIZE);
      ...
      const refResults = await Promise.all(
        batch.map(async (absoluteFilename) => {
          const relativeFilename = make_relative_path(absoluteFilename, workingPath);
          const source = await import_file(absoluteFilename);
          const references = get_type_references_from_source(source, relativeFilename, knownTypeNames);
          return references;
        })
      );

      for (const refs of refResults) {
        allReferences.push(...refs);
      }
    }

    // Convert references to database format
    const dbReferences = allReferences
      .map((ref) => {
        const entity = typeNameToEntity.get(ref.type_name);
    ...
    if (dbReferences.length > 0) {
      const CHUNK_SIZE = 1000;
      for (let i = 0; i < dbReferences.length; i += CHUNK_SIZE) {
        const chunk = dbReferences.slice(i, i + CHUNK_SIZE);
        await batch_insert_references(chunk);
      }
    }

    console.log(`Phase 4 complete: ${Date.now() - refStart}ms - ${dbReferences.length} references`);
  } else {
    console.log(`Phase 4 skipped: No structs/classes found`);
  }

  // Phase 5: Extract and store symbol references (cross-reference browser)
  console.log('Phase 5: Extracting symbol references...');
  const symRefStart = Date.now();

  // Clear existing symbol references before repopulating
  await clear_symbol_references_for_project({ id: project_id });

  // Build a map of definition symbols to entity IDs for linking
  const definitionToEntity = new Map();
  for (const entity of Object.values(known_entities)) {
    // Key by symbol + filename for definitions
    const key = `${entity.symbol}:${entity.filename}`;
    definitionToEntity.set(key, entity);
  }

  const allSymbolReferences = [];

  // Re-parse files to extract all identifiers
  for (let i = 0; i < files.length; i += PARSE_BATCH_SIZE) {
    const batch = files.slice(i, i + PARSE_BATCH_SIZE);

    const symRefResults = await Promise.all(
      batch.map(async (absoluteFilename) => {
        const relativeFilename = make_relative_path(absoluteFilename, workingPath);
        const source = await import_file(absoluteFilename);
        const identifiers = get_all_identifiers_from_source(source, relativeFilename);
        return identifiers;
      })
    );

    for (const refs of symRefResults) {
      allSymbolReferences.push(...refs);
    }
  }

  // Convert to database format and try to link definitions
  const dbSymbolReferences = allSymbolReferences.map((ref) => {
    // Try to find a matching definition entity
    let definition_entity_id = null;
    if (ref.is_definition) {
      // If this is a definition, look up the entity for this symbol/file
      const key = `${ref.symbol}:${ref.filename}`;
      const entity = definitionToEntity.get(key);
      if (entity) {
        definition_entity_id = entity.id;
      }
    }
  ...
  if (dbSymbolReferences.length > 0) {
    const CHUNK_SIZE = 1000;
    for (let i = 0; i < dbSymbolReferences.length; i += CHUNK_SIZE) {
      const chunk = dbSymbolReferences.slice(i, i + CHUNK_SIZE);
      await batch_insert_symbol_references(chunk);
    }
  }

  console.log(`Phase 5 complete: ${Date.now() - symRefStart}ms - ${dbSymbolReferences.length} symbol references`);

  // Phase 6: Extract and store inheritance relationships
  console.log('Phase 6: Extracting inheritance relationships...');
  const inheritStart = Date.now();

  // Clear existing inheritance data before repopulating
  await clear_inheritance_for_project({ id: project_id });

  // Build a map of class/struct entities by symbol for linking
  const classEntitiesBySymbol = new Map();
  for (const entity of Object.values(known_entities)) {
    if (entity.type === 'class' || entity.type === 'struct') {
      // Store by symbol (may have multiple with same name in different files)
      if (!classEntitiesBySymbol.has(entity.symbol)) {
        classEntitiesBySymbol.set(entity.symbol, []);
      }
      classEntitiesBySymbol.get(entity.symbol).push(entity);
    }
  }

  const allInheritanceRelationships = [];

  // Re-parse files to extract inheritance info
  for (let i = 0; i < files.length; i += PARSE_BATCH_SIZE) {
    const batch = files.slice(i, i + PARSE_BATCH_SIZE);

    const inheritResults = await Promise.all(
      batch.map(async (absoluteFilename) => {
        const relativeFilename = make_relative_path(absoluteFilename, workingPath);
        const source = await import_file(absoluteFilename);
        const inheritanceInfo = extract_inheritance_from_source(source, relativeFilename);
        return inheritanceInfo;
      })
    );

    for (const classInfos of inheritResults) {
      for (const classInfo of classInfos) {
        // Find the entity for this class
        const entities = classEntitiesBySymbol.get(classInfo.class_name) || [];
        const entity = entities.find(e => e.filename === classInfo.filename);

        if (entity && classInfo.relationships.length > 0) {
          // Update the entity with inheritance info
          const parentClass = classInfo.relationships.find(r => r.relationship_type === 'extends')?.parent_symbol || null;
          const interfaces = classInfo.relationships
            .filter(r => r.relationship_type === 'implements')
            .map(r => r.parent_symbol);

          await update_entity_inheritance({
            id: entity.id,
          ...
          // Create inheritance relationship records
          for (const rel of classInfo.relationships) {
            // Try to find the parent entity
            const parentEntities = classEntitiesBySymbol.get(rel.parent_symbol) || [];
            const parentEntity = parentEntities.length > 0 ? parentEntities[0] : null;

            allInheritanceRelationships.push({
              child_entity_id: entity.id,
              parent_entity_id: parentEntity?.id || null,
  ...
  // Batch insert inheritance relationships
  if (allInheritanceRelationships.length > 0) {
    const CHUNK_SIZE = 1000;
    for (let i = 0; i < allInheritanceRelationships.length; i += CHUNK_SIZE) {
      const chunk = allInheritanceRelationships.slice(i, i + CHUNK_SIZE);
      await batch_insert_inheritance(chunk);
    }
  }

  console.log(`Phase 6 complete: ${Date.now() - inheritStart}ms - ${allInheritanceRelationships.length} inheritance relationships`);
