...
const create_or_update_project = async ({
  ...
  on_progress = null, // Optional callback: (progress, message) => void
  parser_threads = DEFAULT_THREAD_COUNT // Number of parallel parser threads (default 5)
}) => {
  ...
  // Memory-saving thresholds - more aggressive for very large repos
  const LARGE_REPO_THRESHOLD = 300; // Enable memory-saving mode
  const VERY_LARGE_REPO_THRESHOLD = 1000; // Even more aggressive savings
  const use_memory_saving_mode = files.length > LARGE_REPO_THRESHOLD;
  const use_aggressive_memory_saving = files.length > VERY_LARGE_REPO_THRESHOLD;

  console.log(`Found ${files.length} files to process`);
  if (use_aggressive_memory_saving) {
    console.log(
  ...
  // Phase 1: Parse all files in PARALLEL using worker threads
  // Extract entities, sourcecode, call expressions, comments, symbol references, and inheritance
  // This reads each file exactly once and parses them concurrently
  console.log(
    `Phase 1: Parsing files (parallel with ${parser_threads} threads)...`
  );
  const parse_start = Date.now();

  // Clear references BEFORE we start inserting new ones
  // (moved here from Phase 3 so we can insert incrementally)
  ...
  // For large repos, disable indexes during bulk loading for 5-10x speedup
  ...
  // Set up our known entities - only store minimal info needed for relationships
  let known_entities = {};

  // Accumulators for Phase 2 (relationships)
  let function_calls = [];
  let comments = [];

  // Inheritance needs class entity IDs, so we defer it but it's small (only classes with inheritance)
  let pending_inheritance = []; // Inheritance info (class extends/implements)

  // DB_BATCH_SIZE controls how many files we accumulate before doing a single DB insert
  // Smaller batch size to reduce memory pressure during parallel parsing
  const DB_BATCH_SIZE = 50;
  // For symbol refs, use smaller batches to avoid memory buildup and long-running queries
  // With parallel parsing, refs accumulate quickly so we need frequent flushes
  ...
  // Create parser pool for parallel file parsing
  const parser_pool = create_parser_pool(parser_threads);
  await parser_pool.init();

  // Prepare file list with both absolute and relative paths
  ...
  // Progress update frequency - more frequent for better UX
  const PROGRESS_UPDATE_INTERVAL = 10; // Update progress every N files

  // Process results as they complete
  const process_parse_result = async (result) => {
    if (!result.success) {
      console.warn(
    ...
    // Accumulate sourcecode for batch insert
    ...
    // Collect call expressions and comments for Phase 2
    ...
    const should_flush = batch_sourcecodes.length >= DB_BATCH_SIZE;

    if (should_flush) {
      // Batch insert sourcecodes
      ...
      if (batch_entities.length > 0) {
        const inserted = await batch_insert_or_update_entities(batch_entities);
        total_entities += inserted.length;

        // Store minimal entity info for relationship building
        for (const entity of inserted) {
          ...
          known_entities[entity_key(entity.symbol, entity.filename)] =
            minimal_entity;
      ...
      // Hint to GC after flush
      if (use_memory_saving_mode) {
        try_gc();
      }
    }

    // Report progress frequently for better UX (every PROGRESS_UPDATE_INTERVAL files)
    if (
      on_progress &&
      completed_files - last_progress_update >= PROGRESS_UPDATE_INTERVAL
    ) {
      last_progress_update = completed_files;
      // Phase 1 spans 20-70% of total progress
    ...
    // Batch insert symbol references separately with larger batch size for performance
    const should_flush_symbol_refs =
      batch_symbol_refs.length >= SYMBOL_REF_BATCH_SIZE;
    if (should_flush_symbol_refs && batch_symbol_refs.length > 0) {
  ...
  // Parse all files in parallel and process results as they complete
  await parser_pool.parseFiles(
    file_list,
    project_id,
    async (result, completed, total) => {
      await process_parse_result(result);
    }
  );

  // Terminate the parser pool
  ...
  // Rebuild symbol reference indexes if we disabled them
  if (use_memory_saving_mode) {
  ...
  // Phase 3: Process inheritance (symbol refs already inserted in Phase 1)
  // Type references removed - symbol_reference table already tracks all identifier usage
  ...
  // Build lookup map for class entities
  ...
  // Process inheritance - resolve entity IDs and insert
  let inherit_count = 0;
  ...
  pending_inheritance = null; // Free memory
  ...
  // Batch update inheritance on entities
  ...
  // Clear known_entities to free memory
  known_entities = null;
  try_gc();
