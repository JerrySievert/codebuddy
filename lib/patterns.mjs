...
const DESIGN_PATTERNS = {
  ...
  adapter: {
    ...
    indicators: {
      ...
      java: [
        /class\s+\w*[Aa]dapter\s+implements/,
        /private\s+\w+\s+adaptee/
      ],
      go: [
        /type\s+\w*[Aa]dapter\s+struct/
      ],
      rust: [
        /struct\s+\w*[Aa]dapter/
      ]
    }
  },
  ...
  strategy: {
    ...
    indicators: {
      ...
      go: [
        /type\s+\w*[Ss]trategy\s+interface/
      ],
      rust: [
        /trait\s+\w*[Ss]trategy/
      ]
    }
  },
  ...
  command: {
    ...
    indicators: {
      ...
      go: [
        /type\s+\w*[Cc]ommand\s+interface/,
        /Execute\s*\(\s*\)/
      ],
      rust: [
        /trait\s+\w*[Cc]ommand/,
  ...
  iterator: {
    ...
    indicators: {
      ...
      rust: [
        /impl\s+Iterator\s+for/,
        /fn\s+next\s*\(&mut\s+self\)/
      ]
    }
  },
 ...
 */
const detectPatternsInSource = (source, language) => {
  const detected = [];
  const lang = language.toLowerCase();

  for (const [patternId, pattern] of Object.entries(DESIGN_PATTERNS)) {
    const indicators = pattern.indicators[lang] || pattern.indicators.javascript || [];
 ...
 */
const analyzeErrorHandling = (source, language) => {
  const lang = language.toLowerCase();
  const result = {
    try_catch_count: 0,
    empty_catch_count: 0,
  ...
  const tryCatchPattern = ERROR_HANDLING_PATTERNS.try_catch[lang];
  const emptyCatchPattern = ERROR_HANDLING_PATTERNS.empty_catch[lang];
  const finallyPattern = ERROR_HANDLING_PATTERNS.finally_block[lang];

  if (tryCatchPattern) {
    const matches = source.match(tryCatchPattern);
    result.try_catch_count = matches ? matches.length : 0;
  }

  if (emptyCatchPattern) {
    const matches = source.match(emptyCatchPattern);
    result.empty_catch_count = matches ? matches.length : 0;
  }

  if (finallyPattern) {
    const matches = source.match(finallyPattern);
...
const analyze_project_patterns = async (projectId) => {
  ...
  for (const patternId of Object.keys(DESIGN_PATTERNS)) {
    results.patterns[patternId] = {
      name: DESIGN_PATTERNS[patternId].name,
      category: DESIGN_PATTERNS[patternId].category,
      count: 0,
      locations: []
    };
  }
  ...
  for (const entity of entities) {
    ...
    const detected = detectPatternsInSource(source, language);
    ...
    const errorHandling = analyzeErrorHandling(source, language);
    results.summary.error_handling.total_try_catch += errorHandling.try_catch_count;
    results.summary.error_handling.total_empty_catch += errorHandling.empty_catch_count;
    results.summary.error_handling.total_finally += errorHandling.finally_count;

    if (errorHandling.try_catch_count > 0) {
      results.summary.error_handling.functions_with_error_handling++;
    }

    // Track empty catch blocks as issues
    if (errorHandling.empty_catch_count > 0) {
      results.error_handling_issues.push({
        type: 'empty_catch',
        count: errorHandling.empty_catch_count,
        symbol,
        filename,
        line: start_line,
        severity: 'warning',
        message: `${errorHandling.empty_catch_count} empty catch block(s) - exceptions may be silently ignored`
      });
    }
    ...
    results.by_file[filename].error_handling_issues += errorHandling.empty_catch_count;
  }
  ...
  for (const [id, data] of Object.entries(results.patterns)) {
    if (data.count > 0) {
      activePatterns[id] = data;
    }
  }
  results.patterns = activePatterns;

  // Sort pattern locations by frequency
  results.pattern_locations.sort((a, b) => {
    const countA = results.patterns[a.pattern_id]?.count || 0;
    const countB = results.patterns[b.pattern_id]?.count || 0;
