...
const get_analysis_dashboard = async (project_id) => {
  ...
  const totalFunctions = parseInt(functionStats[0]?.total_functions || 0);
  const deadFunctionCount = parseInt(deadCodeCount[0]?.count || 0);
  const totalFiles = parseInt(fileCount[0]?.count || 0);
  const documented = parseInt(documentedCount[0]?.count || 0);
  const circularCount = parseInt(circularDeps[0]?.count || 0);

  // Calculate simple metrics
  const deadCodePercentage = totalFunctions > 0
    ? Math.round((deadFunctionCount / totalFunctions) * 100)
    : 0;
  const docCoverage = totalFunctions > 0
    ? Math.round((documented / totalFunctions) * 100)
    : 0;

  // Simple health score based on available data
  const healthScore = Math.round(
    (Math.max(0, 100 - deadCodePercentage * 2) + docCoverage) / 2
  );
  ...
  return {
    ...
    summaries: {
      dead_code: {
        dead_function_count: deadFunctionCount,
        dead_code_percentage: deadCodePercentage,
      ...
      api_surface: {
        public_count: totalFunctions,
        private_count: 0,
        documentation_coverage: docCoverage
      },
      documentation: {
        coverage_percentage: docCoverage,
        fully_documented: documented,
        undocumented: totalFunctions - documented
      },
      scope: {
