 ...
 * This provides quick summary counts for the analysis overview.
 * @param {number} project_id - The project ID
...
const calculate_and_store_dashboard = async (project_id) => {
  ...
  const [
    ...
    query`
      SELECT
        ...
        COUNT(*) FILTER (WHERE language IN ('python', 'javascript', 'typescript')) as dynamic_functions,
        AVG(COALESCE(
          CASE WHEN source IS NOT NULL THEN
    ...
    `,
    // Code smells count (functions with high complexity or too many parameters)
    query`
      SELECT COUNT(*) as count
      FROM entity
      WHERE project_id = ${project_id}
        AND type = 'function'
        AND (
          LENGTH(parameters) - LENGTH(REPLACE(parameters, ',', '')) >= 5
          OR LENGTH(source) - LENGTH(REPLACE(source, E'\n', '')) > 100
        )
    `,
    // Security issues count (basic pattern matching for common vulnerabilities)
    query`
      SELECT COUNT(*) as count
      FROM entity
      WHERE project_id = ${project_id}
        ...
        AND (
          source ILIKE '%eval(%'
          OR source ILIKE '%exec(%'
          OR source ILIKE '%shell_exec%'
          OR source ILIKE '%system(%'
          OR source ILIKE '%innerHTML%'
          OR source ILIKE '%dangerouslySetInnerHTML%'
          OR source ILIKE '%sql%' AND source ILIKE '%+%'
        )
    `,
    // Scope issues count (global variables)
    query`
      SELECT COUNT(*) as count
      FROM entity
      WHERE project_id = ${project_id}
        ...
        AND (
          source ILIKE '%global %'
          OR source ILIKE '% global.%'
          OR source ILIKE '%window.%'
        )
    `
  ]);
  ...
  const documented = parseInt(function_stats[0]?.documented_count || 0);
  const circular_count = parseInt(circular_deps[0]?.count || 0);
  const dynamic_functions = parseInt(function_stats[0]?.dynamic_functions || 0);
  const avg_loc = parseFloat(function_stats[0]?.avg_loc || 0);
  const code_smells = parseInt(code_smells_count[0]?.count || 0);
  const security_issues = parseInt(security_count[0]?.count || 0);
  const scope_issues = parseInt(scope_issues_count[0]?.count || 0);
  ...
  const type_coverage =
    total_functions > 0
      ? Math.round(
          ((total_functions - dynamic_functions) / total_functions) * 100
        )
      : 0;

  // Simple maintainability rating based on avg LOC and doc coverage
  const maintainability_index = Math.round(
    Math.max(0, Math.min(100, 100 - avg_loc / 2 + doc_coverage / 2))
  );
  ...
  const dashboard = {
    ...
    scope_issues_count: scope_issues,
