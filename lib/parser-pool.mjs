...
class ParserPool {
  ...
  async init() {
    ...
    for (let i = 0; i < this.thread_count; i++) {
      ...
      const handle_worker_message = (result) => {
      ...
      worker.on('error', handle_worker_error);
      worker.on('exit', handle_worker_exit);
  ...
  _replace_worker(failed_worker) {
    ...
    const handle_replacement_message = (result) => {
      this._handle_worker_result(worker, result);
    };

    const handle_replacement_error = (error) => {
      console.error(`Parser worker error:`, error.message);
      this._replace_worker(worker);
    };

    const handle_replacement_exit = (code) => {
      if (code !== 0 && !this.terminated) {
        console.error(`Parser worker exited with code ${code}`);
        this._replace_worker(worker);
      }
    ...
    worker.on('message', handle_replacement_message);
    worker.on('error', handle_replacement_error);
    worker.on('exit', handle_replacement_exit);
  ...
  parse_file(absolute_filename, relative_filename, project_id) {
    const execute_parse_task = (resolve, reject) => {
      if (this.terminated) {
    ...
    return new Promise(execute_parse_task);
  }
  ...
  async parseFiles(files, project_id, on_file_complete = null) {
    ...
    const process_next = async () => {
      ...
      const handle_parse_result = async (result) => {
        completed_count++;

        // IMPORTANT: Call the callback BEFORE removing from in_flight
        // This ensures the main loop doesn't exit until all callbacks complete
        if (on_file_complete) {
