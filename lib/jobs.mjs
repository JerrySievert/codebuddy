...
const process_next_job = () => {
  ...
  const handle_worker_message = (msg) => {
  ...
  const handle_worker_message_error = (error) => {
  ...
  const handle_worker_exit = (code) => {
    ...
    const schedule_next_job = () => {
      process_next_job();
    };
    setImmediate(schedule_next_job);
  };

  worker.on('message', handle_worker_message);
  worker.on('error', handle_worker_error);
  worker.on('messageerror', handle_worker_message_error);
